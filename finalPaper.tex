...
\maketitle
\begin{abstract}
Ligands can react with a particular receptor in one of three ways: bind and activate the receptor, bind and inhibit the receptor, and not bind at all. Whether a ligand is likely to bind to a receptor can be determined by considering the features such as ligand and receptor shapes, charges, and molecular forces. All of the factors can be accounted via the fred score.

Since the relative positions of the ligand and receptor affect the likelihood of binding, the fred scores for 500 frames for each ligand were taken to help determine whether that particular ligand will bind with the receptor. These 500 frames will be used as the features and will be the feature vector for the ligand instance.

The labeled ligand instances and their 500-fred-score-long feature vectors will be used to train the $\epsilon$-sphere, nearest neighbors algorithm. The algorithm will then be employed on test instances to determine if the test ligand will bind to the given receptor.
\end{abstract}

\section{Introduction}

The $\epsilon$-sphere nearest neighbors algorithm is a close relative of the K-nearest neighbors algorithm; both are clustering algorithms designed with the purpose of categorizing data into a certain number of categories, or labels. As an example, for the ligand-receptor data used, the ligands would be labeled with whether or not they could potentially bind with a given receptor.

Clustering algorithms work by using nearby points to determine which cluster or label a given point should be placed in based on the labels of the points around it. The only difference between above-mentioned $\epsilon$-sphere and K nearest neighbors is the number of points around the test point that is used to help determine the test point's label. In the K nearest neighbors algorithm, the K points closest to the test point are taken, and the test point is assigned the majority label amongst those points; in the $\epsilon$-sphere algorithm, all points that are within a Euclidean distance of $\epsilon$ from the test point are used.

The general structure of the $\epsilon$-sphere clustering algorithm is the same as any machine learning algorithm: teaching the algorithm using a training dataset, and test the accuracy of the algorithm with a development dataset. During training, the training data points are simply read in and stored. Since the training data is already labeled, the training data points are already separated into clusters.

During testing, unlabeled data points are inputted individually, and the algorithm attempts to place them in a cluster. It does so by taking all data points within $\epsilon$ of the test point and calculating the majority label amongst those points. The test point is then assigned to that majority label and added to the cluster, so the test point will influence the labels of future test points. Testing continues as more test points are labeled and added to the clusters.
